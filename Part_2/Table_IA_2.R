...
create_alternate_sims <- function(input_filename, drop_top_bottom = 10, at_cols = NULL, se_cols = NULL, 
  ...
  mydata <- read_csv(paste(INPUT_DIRECTORY, input_filename, sep="/"), col_names=TRUE)
  ...
  if(WRITE_TO_FILE){
    write.csv(mydata, row.names=FALSE, file=paste(OUTPUT_DIRECTORY, output_filename, sep="/"))
  }
...
create_alternate_bias_table <- function(input_filename,  at_cols = NULL, se_cols = NULL,
  ...
  if(WRITE_TO_FILE){
    write.csv(out_df, row.names=FALSE, file=paste(OUTPUT_DIRECTORY, output_filename, sep="/"))
  }
  ...
  rv <- xtable(out_mc, digits=2, caption = "")
  if(WRITE_TO_FILE){
    print(rv, include.rownames=FALSE, file=paste(OUTPUT_DIRECTORY, output_filename, sep="/"))
  }
  rv
}

for(csv_file in list.files(INPUT_DIRECTORY, pattern="\\_sims_alt.csv$")){
