...
data <- subset(data, data$`Cash Flow win` < 0)
data <- subset(data, data$`Macro q win` > 5.63)
data <- data %>%
...
print(xtable(RDD_panel,digits=c(0, 2, 2, 2, 4, 0, 0)), include.rownames = FALSE,
      file = "output/Table_6_Panel_D.txt")
