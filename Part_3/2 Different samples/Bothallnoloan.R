...
##select only variables we need and stack these on top of each other:
# keep track of covenant type since we include both
fc_link_table$type = "CR"
nw_table_with_slack$type = "NW"
alldata_colnames <- intersect(names(nw_table_with_slack),
                              names(fc_link_table))
alldata <- rbind.fill(nw_table_with_slack[alldata_colnames],
                      fc_link_table[alldata_colnames])


######Get rid of loans with negative initial slack:

  dim(alldata)
alldata <- subset(alldata, initial_slack >= 0)
  dim(alldata)
...
# fc_link_table_min_slack <- alldata %>%
#   group_by(gvkey, datadate) %>%
#   transmute(min_slack = min(slack, na.rm=TRUE)) %>%
#   ungroup()
# 
# ##merges the minimum slack back into the data:
# fc_link_table_min_slack <- merge(fc_link_table_min_slack, alldata, by = c("gvkey", "datadate"), all = TRUE)
...
#############
###Save data:
save(tmp, file = paste(project.dir, "Samples/bothallnoloan.Rdata", sep = "/"))
