...
nw_table_with_slack <- subset(nw_table_with_slack, initial_slack >= 0)
  dim(nw_table_with_slack)

###So we have two data sets with min_slack, fc_link_table_min_slack and nw_table_min_slack

# nw_table_min_slack <- nw_table_with_slack %>%
#   group_by(gvkey, datadate) %>%
#   transmute(min_slack = min(slack, na.rm=TRUE)) %>%
#   ungroup()
# 
# nw_table_min_slack <- merge(nw_table_min_slack, nw_table_with_slack, by = c("gvkey", "datadate"), all = TRUE)
...
print(dim(nw_table_min_slack))

##To keep minimum slack:  (but there could be more than one facility at the minimum)
nw_table_min_slack <- filter(nw_table_min_slack, slack == min_slack)

print(dim(nw_table_min_slack))

# only keep the first observation for each gvkey datadate pair
nw_table_min_slack <- nw_table_min_slack[!duplicated(nw_table_min_slack[c("gvkey", "datadate")]),]

print(dim(nw_table_min_slack))


tmp <- inner_join(nw_table_min_slack, fundq_extended)
print(dim(nw_table_min_slack))
...
#############
###Save data:
save(tmp, file = paste(project.dir, "Samples/NWallnoloan.Rdata", sep = "/"))
